...
@Component({
  selector: 'app-inscription-collaborator-edit-member',
  templateUrl: './inscription-collaborator-edit-member.component.html'
})
export class InscriptionCollaboratorEditMemberComponent implements OnInit {
    ...
    public prepareEdit() {
        ...
        this.item.renewDate = this.service.format(this.item.renewDate);
    }
 ...
 public edit(): void {
    ...
    public validateInscriptionMembres(){
   ...
   public addInscriptionMembres() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.inscriptionMembresElement.id == null){
                this.item.inscriptionMembres.push(this.inscriptionMembresElement);
            }else{
                const index = this.item.inscriptionMembres.findIndex(e => e.id == this.inscriptionMembresElement.id);
                this.item.inscriptionMembres[index] = this.inscriptionMembresElement;
            }
   ...
   public async openCreateInscriptionCollaboratorState(inscriptionCollaboratorState: string) {
        const isPermistted = await this.roleService.isPermitted('InscriptionCollaboratorState', 'edit');
        if (isPermistted) {
   ...
   public async openCreateInscriptionCollaboratorType(inscriptionCollaboratorType: string) {
        const isPermistted = await this.roleService.isPermitted('InscriptionCollaboratorType', 'edit');
        if (isPermistted) {
