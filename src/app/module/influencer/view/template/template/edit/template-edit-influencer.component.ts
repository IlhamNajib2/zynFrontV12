...
@Component({
  selector: 'app-template-edit-influencer',
  templateUrl: './template-edit-influencer.component.html'
})
export class TemplateEditInfluencerComponent implements OnInit {
    ...
    public prepareEdit() {
        this.item.addingDate = this.service.format(this.item.addingDate);
        this.item.lastUpdateDate = this.service.format(this.item.lastUpdateDate);
    }
 ...
 public edit(): void {
   ...
   public async openCreateLevelTemplate(levelTemplate: string) {
        const isPermistted = await this.roleService.isPermitted('LevelTemplate', 'edit');
        if (isPermistted) {
   ...
   public async openCreateTechnology(technology: string) {
        const isPermistted = await this.roleService.isPermitted('Technology', 'edit');
        if (isPermistted) {
   ...
   public async openCreateCategoryTemplate(categoryTemplate: string) {
        const isPermistted = await this.roleService.isPermitted('CategoryTemplate', 'edit');
        if (isPermistted) {
   ...
   public async openCreateDomainTemplate(domainTemplate: string) {
        const isPermistted = await this.roleService.isPermitted('DomainTemplate', 'edit');
        if (isPermistted) {
